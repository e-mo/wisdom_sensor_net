...
teros_return teros_init(
	...
	*_teros = malloc(sizeof(teros));
	...
	if(_teros == NULL) return malloc_null;
	//save some struct elements to reference in other functions
	(*_teros)->serial = serial;
	(*_teros)->model = model;
	(*_teros)->uart_tx_pin = uart_tx_pin;
	(*_teros)->uart_rx_pin = uart_rx_pin;
	(*_teros)->pwr_pin = pwr_pin;
	(*_teros)->substrate_type = substrate_type;
...
teros_return teros_get_data(teros *teros, teros_data *data) {
	...
	if(str[0] == '\t') { //just be absolutely certain its correct and starts with a tab
		...
		return read_unsuccessful; //if its not a tab, bad read
	}

	if(crc != _crc6_validate((const char *)&str)) {
		return invalid_check_crc; //validate the checksum of the string
	}
	if(check != _checksum_validate((const char *)&str)) return invalid_check_crc; //same with crc6
